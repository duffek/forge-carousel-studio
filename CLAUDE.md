...
- **Themes**: per-story brand themes (`meta.theme`: `foundersforge | avante | summit`), specs sourced from David's Canva brand kits. `lib/themes.ts` holds ids/wordmarks/swatches/image-style hints; all visuals are CSS variable overrides in `app/globals.css` scoped to `.slide[data-theme=...]` (`--slide-bg`, `--accent-fill`, `--em-color`, `--slide-font-display/body`, divider/stripe motifs). Theme fonts (Poppins = Galano substitute for Avante; Anton for Summit) are self-hosted like the others. Adding a theme = new block in globals.css + entry in `THEMES` + wordmark in `public/images/`.
